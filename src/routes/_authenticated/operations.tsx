...
import {
  ...
  Package, UserCircle2, Search, UserX, ChevronDown, MessageSquare, X, Pencil, Trash2,
} from "lucide-react";
...
function OperationsPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      {notesColis && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/50 px-4 py-10 backdrop-blur-sm" onClick={() => { setNotesColis(null); void refreshColis(); }}>
          <div className="w-full max-w-2xl overflow-hidden rounded-2xl border border-border bg-card shadow-card" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between gap-3 border-b border-border px-5 py-4">
              <div className="flex min-w-0 items-center gap-2">
                <MessageSquare className="h-5 w-5 shrink-0 text-primary" /><h2 className="shrink-0 font-bold">Notes internes</h2>
                <span className="truncate rounded-md bg-info/15 px-2 py-0.5 font-mono text-sm font-bold text-info">{notesColis.tracking}</span>
              </div>
              <button onClick={() => { setNotesColis(null); void refreshColis(); }} className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground"><X className="h-5 w-5" /></button>
            </div>
